...
import { Search, UserX, UserCheck, ShieldAlert, Filter, Upload, FileSpreadsheet, Users } from 'lucide-react'
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            if (rollIdx === -1 || nameIdx === -1 || branchIdx === -1) {
                return toast.error("CSV must contain 'Roll', 'Name', and 'Branch' columns")
            }
            ...
            const records = rows.slice(1).map(row => {
                ...
                return {
                    ...
                    degree: degreeIdx !== -1 ? cols[degreeIdx] : '',
            ...
            <div className='glass-panel p-6 rounded-3xl mb-8 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 shadow-sm border border-gray-100'>
                <div>
                    <h2 className='text-2xl font-bold text-gray-800 tracking-tight'>
                        {activeTab === 'registered' ? 'Student Directory' : 'Official Batch Ledger'}
                    </h2>
                    <p className='text-gray-500 text-sm mt-1'>
                        {activeTab === 'registered' ? 'Manage registered candidates and enforce strict disciplinary actions.' : 'Upload official CSVs and track the entire enrolled branch.'}
                    </p>
                </div>
                    ...
                    <table className='w-full text-sm text-left whitespace-nowrap'>
                        <thead className='bg-gray-50/80 border-b border-gray-100 text-gray-600 font-bold uppercase tracking-wider text-xs'>
                            <tr>
                                ...
                                <th className='py-4 px-6 text-center'>{activeTab === 'ledger' ? 'Verification' : 'Quick Action'}</th>
                            </tr>
                        </thead>
                        <tbody className='divide-y divide-gray-100'>
                            ...
                            {((activeTab === 'registered' && filteredStudents.length === 0) || (activeTab === 'ledger' && studentRecords.length === 0)) && (
                                <tr>
